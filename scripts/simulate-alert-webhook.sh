...
# Console schema to the local AlertWebhookController, so the receiver gets
# exactly the bytes Camunda SaaS would deliver.
#
# Usage:
...
#
# Env vars (optional):
#   APP_ALERTS_WEBHOOK_HMAC_SECRET  if set, signs the body with HMAC-SHA256 and
#                                   sends X-Camunda-Signature-256
...
TMP=$(mktemp -t alert-payload.XXXXXX)
trap 'rm -f "$TMP"' EXIT
printf '%s' "$PAYLOAD" > "$TMP"

if [[ -n "${APP_ALERTS_WEBHOOK_HMAC_SECRET:-}" ]]; then
  SIG=$(openssl dgst -sha256 -hmac "$APP_ALERTS_WEBHOOK_HMAC_SECRET" -hex < "$TMP" \
        | awk '{print $NF}')
  echo "POST $RECEIVER (signed)" >&2
  curl -sS -o /dev/null -w "HTTP %{http_code}\n" -X POST "$RECEIVER" \
    -H "Content-Type: application/json" \
    -H "X-Camunda-Signature-256: $SIG" \
    --data-binary @"$TMP"
else
  echo "POST $RECEIVER (unsigned)" >&2
  curl -sS -o /dev/null -w "HTTP %{http_code}\n" -X POST "$RECEIVER" \
    -H "Content-Type: application/json" \
    --data-binary @"$TMP"
fi
